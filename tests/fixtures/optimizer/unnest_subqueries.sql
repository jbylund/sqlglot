...
# title: correlated NOT IN is declined: the anti-join rewrite loses three-valued NULL semantics
...
# title: NOT NOT IN unnests like a plain IN
SELECT * FROM x WHERE NOT NOT (x.a IN (SELECT y.a AS a FROM y));
SELECT * FROM x LEFT JOIN (SELECT y.a AS a FROM y GROUP BY y.a) AS _u_0 ON x.a = _u_0.a WHERE NOT NOT (NOT _u_0.a IS NULL);
...
SELECT * FROM x LEFT JOIN (SELECT 1 AS _u_1, y.a AS _u_2 FROM y GROUP BY y.a) AS _u_0 ON _u_0._u_2 = x.a OR _u_0._u_2 IS NULL WHERE _u_0._u_1 IS NULL;

# title: uncorrelated EXISTS in WHERE unnests into a LEFT JOIN ON TRUE
SELECT * FROM x WHERE EXISTS (SELECT 1 FROM y);
SELECT * FROM x LEFT JOIN (SELECT DISTINCT 1 FROM y) AS _u_0 ON TRUE WHERE NOT _u_0."1" IS NULL;

# title: uncorrelated NOT EXISTS in WHERE unnests into a LEFT JOIN ON TRUE
SELECT * FROM x WHERE NOT EXISTS (SELECT 1 FROM y WHERE y.b > 0);
SELECT * FROM x LEFT JOIN (SELECT DISTINCT 1 FROM y WHERE y.b > 0) AS _u_0 ON TRUE WHERE NOT NOT _u_0."1" IS NULL;
...
# title: NOT IN is rewritten into a null-safe anti-EXISTS
SELECT * FROM x WHERE x.a NOT IN (SELECT y.a AS a FROM y);
SELECT * FROM x LEFT JOIN (SELECT 1 AS _u_3, _u_0.a AS _u_4 FROM (SELECT y.a AS a FROM y) AS _u_0 GROUP BY _u_0.a) AS _u_2 ON _u_2._u_4 = x.a OR _u_2._u_4 IS NULL OR x.a IS NULL WHERE _u_2._u_3 IS NULL;

# title: NOT IN with UNION ALL subquery is rewritten into a null-safe anti-EXISTS
SELECT * FROM x WHERE x.a NOT IN (SELECT y.a AS a FROM y UNION ALL SELECT z.a AS a FROM z);
SELECT * FROM x LEFT JOIN (SELECT 1 AS _u_3, _u_0.a AS _u_4 FROM (SELECT y.a AS a FROM y UNION ALL SELECT z.a AS a FROM z) AS _u_0 GROUP BY _u_0.a) AS _u_2 ON _u_2._u_4 = x.a OR _u_2._u_4 IS NULL OR x.a IS NULL WHERE _u_2._u_3 IS NULL;
