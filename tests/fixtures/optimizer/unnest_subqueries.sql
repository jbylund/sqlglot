...
# title: NOT IN is rewritten into null-safe scalar COUNT legs
SELECT * FROM x WHERE x.a NOT IN (SELECT y.a AS a FROM y);
SELECT * FROM x CROSS JOIN (SELECT COUNT(*) AS _count FROM (SELECT y.a AS a FROM y) AS _u_0) AS _u_3 LEFT JOIN (SELECT COUNT(*) AS _count, _u_1.a AS _u_5 FROM (SELECT y.a AS a FROM y) AS _u_1 WHERE TRUE GROUP BY _u_1.a) AS _u_4 ON _u_4._u_5 = x.a CROSS JOIN (SELECT COUNT(*) AS _count FROM (SELECT y.a AS a FROM y) AS _u_2 WHERE _u_2.a IS NULL) AS _u_6 WHERE (x.a IS NOT NULL OR _u_3._count = 0) AND (COALESCE(_u_4._count, 0) = 0 AND _u_6._count = 0);

# title: NOT IN with UNION ALL subquery is rewritten into null-safe scalar COUNT legs
SELECT * FROM x WHERE x.a NOT IN (SELECT y.a AS a FROM y UNION ALL SELECT z.a AS a FROM z);
SELECT * FROM x CROSS JOIN (SELECT COUNT(*) AS _count FROM (SELECT y.a AS a FROM y UNION ALL SELECT z.a AS a FROM z) AS _u_0) AS _u_3 LEFT JOIN (SELECT COUNT(*) AS _count, _u_1.a AS _u_5 FROM (SELECT y.a AS a FROM y UNION ALL SELECT z.a AS a FROM z) AS _u_1 WHERE TRUE GROUP BY _u_1.a) AS _u_4 ON _u_4._u_5 = x.a CROSS JOIN (SELECT COUNT(*) AS _count FROM (SELECT y.a AS a FROM y UNION ALL SELECT z.a AS a FROM z) AS _u_2 WHERE _u_2.a IS NULL) AS _u_6 WHERE (x.a IS NOT NULL OR _u_3._count = 0) AND (COALESCE(_u_4._count, 0) = 0 AND _u_6._count = 0);
