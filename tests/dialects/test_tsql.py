...
class TestTSQL(Validator):
    ...
    def test_tsql(self):
        ...
        # only the offset gets attached to the set operation, so the order and a trailing FETCH
        ...
        # a TOP limits only the branch it sits on, so unlike a FETCH it stays inside
        self.validate_identity(
            "SELECT a FROM x UNION ALL SELECT TOP 1 a FROM y ORDER BY a OFFSET 1 ROWS",
            "SELECT * FROM (SELECT a FROM x UNION ALL SELECT TOP 1 a FROM y) AS _l_0 ORDER BY a OFFSET 1 ROWS",
        )
        self.validate_all(
